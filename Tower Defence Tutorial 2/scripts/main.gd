...
func _spawn_wave():
	print("enemy_waves[w]1")
	print("enemy_waves[w]")
	print(enemy_waves)
	for w in range(enemy_waves.size()):
		
		
		var enemy_wave:Array[EnemySettings] = enemy_waves[w].enemies
		
		for i in range(enemy_wave.size()):
			var wave = enemy_wave[i]
			await  get_tree().create_timer(wave.next_enemy_delay).timeout
			print(wave.next_enemy_delay)
			print("instantiating enemy")
			
			var enemy2:Enemy = basic_enemy.instantiate()
			enemy2.enemy_settings = wave
...
func _complete_grid():
	for x in range(PathGenInstance.path_config.width):
		for y in range(PathGenInstance.path_config.height):
